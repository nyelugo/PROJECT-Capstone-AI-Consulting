...
| **13** | Transparency and information for deployers | **Met.** `mvp/mvp_documentation.md` states capabilities, limits, the guard ladder and seven known limitations. The UI shows the reasoning per decision, not just the outcome | — |
...
| **15** | Accuracy, robustness, cybersecurity | **Partial.** Robustness is good: 27 guard cases, no unhandled exception path, graceful degradation when the model or monitoring is unavailable. **Accuracy is explicitly not established** — 60.5% is agreement with complainant-chosen labels | Accuracy metric requires Phase 0. Penetration testing and secrets rotation are Phase 2 |
