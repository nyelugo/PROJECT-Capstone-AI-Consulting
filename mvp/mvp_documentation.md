...
Opens on <http://localhost:8502>. On macOS, double-click **`Assist.app`** on the Desktop
...
| Nav section | Page | Use case | Proposes | Grounded in |
|---|---|---|---|---|
| **Overview — no model involved** | Your complaint book | — | Nothing. **There is no AI on this page** | Arithmetic over public data |
| **Assist — it proposes, you decide** | Triage a complaint | UC-1 | A routing queue and team | A verbatim quote from the complaint |
...
| **Record** | Decision log | — | Nothing | What the system proposed, beside what the person did |

The section headings are not decoration. A CEO leaving the room believing the dashboard's
figures were generated by a model would be a worse outcome than not showing them at all, so
the navigation says which pages involve one and which do not.
...
├── ui.py                    ONE renderer for all three capabilities
├── app_pages/               triage · reporting · anomaly · decision_log
